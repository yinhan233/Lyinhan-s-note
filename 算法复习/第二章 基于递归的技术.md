...
空间复杂度:O(n)
### Horner's规则
基本思想 首先观察多项式： $$P_n(x) = a_nx^n + a_{n-1}x^{n-1} + \dots + a_1x + a_0$$ 
将其变形为嵌套求值形式： $$P_n(x) = ((((a_nx + a_{n-1})x + a_{n-2})x + a_{n-3})\dots x + a_1)x + a_0$$  
这种求值的安排称为Horner规则
导出: 
$$P_{n-1}(x) = a_n x^{n-1} + a_{n-1}x^{n-2} + \dots + a_2 x + a_1$$ 得到以下更有效的方法：   
$$p_n(x) = x p_{n-1}(x) + a_0,\ p_j(x) = x p_{j-1}(x) + a_{n-j}$$  
`伪代码:`  
...
3. $w=\text{SPLIT}(A[\text{low}..\text{high}])$ { $w$为$A[\text{low}]$的新位置 } 
...
时间复杂度是： $$T(n)= \begin{cases} d & \text{if } n=1 \\ 3T(n/2)+bn & \text{if } n>1 \end{cases}$$   
递推式的解是： $T(n) = \Theta(n^{\log_2 3}) = O(n^{1.59})$  
### 矩阵乘法
#### 分治
...
# 动态规划
## 基本概念
<font color="#4bacc6">思想</font>:和分治法一样，把待求解问题分成若干个子问题，与分治法不同，子问题与子问题之间并不独立，并且这些子问题的答案会被保存，在有需要的时候再次利用，从而避免大量重复运算
解题思路:找状态转移方程和边界条件
如何判断是否适用于动态规划，考虑如下三个要素:  
- 重复子问题
- 最优子结构
- 无后效性
## 实际应用
### 背包问题(0-1背包)
<font color="#2DC26B">描述</font>:给出有$n$项物品的$U$，我们要找出一个子集合$S \subseteq U$使得$\sum_{u_i \in S} v_i$ 在约束条件$\sum_{u_i \in S} s_i \leq C$下最大。  
<font color="#f79646">思想</font>:
用子问题定义状态：即$V[i][v]$表示前$i$件物品放入一个容量为$v$的背包可以获得的最大价值。其状态转移方程便为： 
$$V[i][v] = \max\left(V[i-1][v],\ V[i-1][v-s_i] + v_i\right)$$  
针对“将前$i$件物品放入容量为$v$的背包中”这个子问题，若只考虑第$i$件物品的策略（放或不放），那么就可以转化为一个只牵扯前$i-1$件物品的问题：   
- 如果不放入第$i$件物品，那么问题就转化为“前$i-1$件物品放入容量为$v$的背包中”，价值为$V[i-1][v]$；   
- 如果放入第$i$件物品，那么问题就转化为“前$i-1$件物品放入剩余容量为$v-s_i$的背包中”，此时能获得的最大价值就是$V[i-1][v-s_i]$再加上通过放入第$i$件物品获得的价值$v_i$。    
状态转移方程如下:  
$$V[i,j]= \begin{cases} 0 & \text{if } i=0 \text{ 或 } j=0 \\ V[i-1,j] & \text{if } j < s_i \\ \max(V[i-1,j],V[i-1,j-s_i]+v_i) & \text{if } i>0 \text{ 且 } j \ge s_i \end{cases}$$  
`伪代码:`  
输入: 物品集合$U=\{u_1,u_2,...,u_n\}$，体积分别为$s_1,s_2,...,s_n$，价值分别为$v_1,v_2,...,v_n$，容量为$C$的背包。  
输出: $\sum_{u_i \in S} v_i$的最大总价值（满足$\sum_{u_i \in S} s_i \le C$，其中$S \subseteq U$）。 
...
10. if $s_i \le j$ then $V[i,j] \leftarrow \max(V[i,j], V[i-1,j-s_i]+v_i)$ 
11. end for 
12. end for 
13. return $V[n,C]$  
时间复杂度:O(nC)
空间复杂度:O(C)
### 最长公共子序列问题
<font color="#f79646">描述</font>:在字母表$\Sigma$上，分别给出两个长度为$n$和$m$的字符串$A$和$B$，确定$A$和$B$中最长公共子序列的长度原始PRIM。     
定义$A=a_1a_2…a_n$的子序列是一个形式为$a_{i_1}a_{i_2}…a_{i_k}$的字符串，其中每个$j$都在1和$n$之间，并且$1 \le i_1 < i_2 < … < i_k \le n$。  
思路:用表格存储这两个序列的前缀的最长子序列，每次对位置上的数字进行判断，如果相等+1，不相等，取前面较大的那个。    
状态转移方程:  
$$d[i,j]= \begin{cases} 0 & \text{if } i=0 \text{ 或 } j=0, \\ d[i-1,j-1]+1 & \text{if } i,j>0 \text{ 且 } x_i = y_j, \\ \max(d[i,j-1],d[i-1,j]) & \text{if } i,j>0 \text{ 且 } x_i \neq y_j. \end{cases}$$  
`伪代码:`  
输入: 某字母表$\Sigma$上的两个字符串$A$和$B$，长度分别为$n$和$m$   
输出: $A$和$B$的最长公共子序列的长度。   
1. for $i \leftarrow 0$ to $n$ // $i=0$时，$L[i][j]=0$ 
2. $L[i][0] \leftarrow 0$ 
3. end for 
4. for $j \leftarrow 0$ to $m$ // $j=0$时，$L[i][j]=0$ 
5. $L[0][j] \leftarrow 0$ 
6. end for 
...
空间复杂度:O(min{m,n})  
### 矩阵链相乘
描述:给定$n$个矩阵$\{A_1,A_2,\dots,A_n\}$，其中$A_i$与$A_{i+1}$是可乘的，$i=1,2,\dots,n-1$。 计算这$n$个矩阵的连乘积$A_1A_2\cdots A_n$。   
由于矩阵乘法满足结合律，故计算矩阵的连乘积可以有许多不同的计算次序，这种计算次序可以用加括号的方式来确定。若一个矩阵连乘积的计算次序完全确定，也就是说该连乘积已完全加括号，则可以依此次序反复调用2个矩阵相乘的标准算法计算出矩阵连乘积。
状态转移方程:
$$m[i,j]= \begin{cases} 0 & \text{if } i = j \\ \min\limits_{i \le k < j} \left\{ m[i,k] + m[k+1,j] + p[i-1]p[k]p[j] \right\} & \text{if } i < j \end{cases}$$  
`伪代码`:  
输入:p=p[0],p[1]...p[n]   
输出:最佳计算次序s和辅助表m  
1. n ← length[p] - 1 // n个矩阵 
...
8. m[i,j] ← ∞ 
9. for k ← i to j - 1 
10. q ← m[i,k] + m[k+1,j] + p[i-1]p[k]p[j] // 矩阵乘法次数 
11. if q < m[i,j] // 取最小次数 
12. then m[i,j] ← q, s[i,j] ← k 
13. end if 
14. end for 
15. end for 
16. end for
...
空间复杂度:$O(n^2)$  
### Floyd
状态转移方程:$$d^k_{ij}= \begin{cases} I[i,j] & \text{if } k=0 \\ \min\left\{d^{k-1}_{ij},\ d^{k-1}_{ik} + d^{k-1}_{kj}\right\} & \text{if } 1 \le k \le n \end{cases}$$  
输入: 有向图$G$的$n×n$矩阵$I[1..n,1..n]$（$I[i,j]$是$i$到$j$的路径长度），$n$为顶点数。   
输出: 矩阵$D$，使得$D[i,j]$是$i$到$j$的最短距离。 
1. $D \leftarrow I$ // 将输入矩阵$I$复制到$D$ 
